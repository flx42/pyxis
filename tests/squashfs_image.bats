...
@test "--container-save absolute path" {
    ...
    run_srun --container-image=ubuntu:20.04 --container-save=${image} sh -c 'echo pyxis > /test' ; sleep 10s
    run_srun --container-image=${image} --container-save=${image} sh -c 'echo slurm > /test2' ; sleep 10s
    run_srun --container-image=${image} --container-save=${image} --container-remap-root sh -c 'apt-get update && apt-get install -y file' ; sleep 10s
